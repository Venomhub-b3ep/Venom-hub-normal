...
-- Fundo preto e branco gradiente animado
...
Gradient.Color = ColorSequence.new{
    ...
    ColorSequenceKeypoint.new(0.3, Color3.fromRGB(150, 150, 150)), -- Cinza mÃ©dio
    ColorSequenceKeypoint.new(0.7, Color3.fromRGB(50, 50, 50)),   -- Cinza escuro
...
Bar.BackgroundColor3 = Color3.fromRGB(255, 255, 255)  -- Branco puro
Bar.Parent = BarBG
...
-- Adicionando borda branca na foto
local AvatarBorder = Instance.new("UIStroke")
AvatarBorder.Color = Color3.fromRGB(255, 255, 255)
...
task.spawn(function()
    ...
    while ScreenGui.Parent do
        ...
        local grayValue = 150 + 105 * math.sin(t*2)
        Nick.TextColor3 = Color3.fromRGB(
            math.clamp(grayValue, 100, 255),
            math.clamp(grayValue, 100, 255),
            math.clamp(grayValue, 100, 255)
        )
        task.wait(0.03)
...
task.spawn(function()
    ...
    while ScreenGui.Parent do
        ...
        local grayValue = 200 + 55 * math.sin(t*1.5)
        Label.TextColor3 = Color3.fromRGB(
            math.clamp(grayValue, 150, 255),
            math.clamp(grayValue, 150, 255),
            math.clamp(grayValue, 150, 255)
        )
...
-- Texto do jogo que estÃ¡ entrando
...
-- FunÃ§Ã£o para criar partÃ­culas interativas
local function CreateParticle(position)
    local p = Instance.new("Frame")
    p.Size = UDim2.new(0, math.random(3,7), 0, math.random(3,7))
    p.Position = position
    local grayValue = math.random(150, 255)
    p.BackgroundColor3 = Color3.fromRGB(grayValue, grayValue, grayValue)
    p.BackgroundTransparency = 0.3
    p.AnchorPoint = Vector2.new(0.5,0.5)
    p.Parent = Frame
    
    -- Adicionando bordas arredondadas nas partÃ­culas
...
-- CriaÃ§Ã£o automÃ¡tica de partÃ­culas de fundo
for i=1,50 do
    local x = math.random()
    local y = math.random()
...
for i=1,30 do
    ...
    spark.BackgroundColor3 = Color3.fromRGB(255, 255, 255)  -- Branco
    spark.BackgroundTransparency = 0.6
    spark.Parent = Frame
    
    -- Adicionando bordas arredondadas nas explosÃµes
...
Sound.Parent = Frame
Sound.TimePosition = 0  -- garante que comece do inÃ­cio
Sound:Play()
...
local redzlibContent = game:HttpGet("https://pastefy.app/6MhDrVRp/raw", true)
...
local Window = redzlib:MakeWindow({
    Title = Translator:traduzir("ðŸ˜ˆVenom HubðŸ˜ˆ"),
    SubTitle = "By:King",
...
Window:AddMinimizeButton({
	Button = {
		Image = "rbxassetid://129859177292154"
	},
	Corner = {
...
local args = {
    [1] = "RolePlayName",
    [2] = "UsuÃ¡rio do Hub: [Venom Hub]"
}

game:GetService("ReplicatedStorage").RE:FindFirstChild("1RPNam1eTex1t"):FireServer(unpack(args))
local function copyDiscordLink()
    setclipboard ("https://discord.gg/5M42ZXeJA")
end

local Tabs = {
    M = Window:MakeTab({Translator:traduzir("informaÃ§Ãµes"), "Info"}),
    H = Window:MakeTab({Translator:traduzir("InÃ­cio"), "coffee"}),
    PR = Window:MakeTab({Translator:traduzir("ProteÃ§Ã£o"), "shield"}),
    ...
    CFG = Window:MakeTab({Translator:traduzir("ConfiguraÃ§Ãµes"), "Settings"})
}

Tabs.M:AddDiscordInvite({
    Name = Translator:traduzir("Venom Hub"),
    Description = Translator:traduzir("Entre na Nossa Comunidade Oficial do Discord"),
    Logo = "rbxassetid://129859177292154",
    Invite = "https://discord.gg/N3DudhyS8"
})

local function links()
    setclipboard("https://discord.gg/N3DudhyS8")
end
...
do
Tabs.M:AddSection({Translator:traduzir("Ãnuncios <announcement>")})
Tabs.M:AddButton({Translator:traduzir("Aviso Externo"), function()
Window:Dialog({
    Title = "Erros",
    Text = Translator:traduzir("â€¢ O script pode apresentar erros. Caso encontre algum, por favor, reporte-o no nosso Discord."),
    Options = {
...
Window:Dialog({
    Title = "Update",
    Text = Translator:traduzir("â€¢Devido Ã s atualizaÃ§Ãµes, algumas funÃ§Ãµes podem parar de funcionar. Estamos trabalhando para garantir a melhor experiÃªncia."),
    Options = {
        {"Link Discord", function()
            copyDiscordLink()
        end},
...
local Section = Tabs.M:AddSection({Translator:traduzir("ResoluÃ§Ãµes Da Tela")})

        Tabs.M:AddButton({Translator:traduzir("ResoluÃ§Ã£o Normal"), 
          function()
          getgenv().Resolution = {
            [".gg/scripters"] = 1.0
        }
...
getgenv().gg_scripters = "Aori0001"
        ...
        Tabs.M:AddButton({Translator:traduzir("ResoluÃ§Ã£o 0.70"),
          function()
            getgenv().Resolution = {
        [".gg/scripters"] = 0.70
    }
...
getgenv().gg_scripters = "Aori0001"
        ...
        Tabs.M:AddButton({Translator:traduzir("ResoluÃ§Ã£o 0.80"), 
          function()
                    getgenv().Resolution = {
            [".gg/scripters"] = 0.80
        }
...
Tabs.H:AddSection({Translator:traduzir("Status AtualizaÃ§Ã£o")})
    ...
    while true do
        ...
        if scout then
            ...
            if hrpScout and hrpLocal then
                ...
                texto = texto .. string.format("estÃ¡ no servidor | DistÃ¢ncia <%d>", distancia)
            else
                texto = texto .. "estÃ¡ no servidor | DistÃ¢ncia <???>"
            end
        else
            texto = texto .. "nÃ£o estÃ¡ no servidor"
        end
